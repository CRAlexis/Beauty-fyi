...
class ServiceController {
    ...
    async getServiceImage({ request, session, response }) {
        ...
        const query = await Database.table('services').where('id', serviceID).first()
        let image;
        switch (index) {
    ...
    async getServiceData({ request, session, response }) {
        ...
        try {
            const query = await Database.table('services').where('user_id', userID).where("active", 1).paginate(row, 3)
        ...
        try {
            ...
            if (serviceTime > 179) { hours = 3; minutes = serviceTime - 180 } else
                if (serviceTime > 119) { hours = 2; minutes = serviceTime - 120 } else
                    if (serviceTime > 59) { hours = 1; minutes = serviceTime - 60 } else { return { time: serviceTime + " minutes" } }
